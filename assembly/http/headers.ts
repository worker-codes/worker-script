...
export class Headers {
    ...
    encode(writer: Writer): void {
        ...
        let keys = this.headerMap.keys();
        writer.writeArraySize(keys.length);
        for (var i = 0; i < keys.length; i++) {

            writer.writeArraySize(2);
            let key = keys[i];
            let values = this.headerMap.get(key);
            writer.writeString(key);
            writer.writeString(values.join(", "));
            // writer.writeArraySize(values.length);
        ...
        // writer.writeArray(this.headerMap, (writer: Writer, item: string) => {
